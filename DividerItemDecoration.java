...
import com.zhenai.widget.utils.DisplayUtil;
...
/*
* item最好指定高度  以及item宽度使用match_parent
* */
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public static final int SPE_LIST = 3;
    ...
    public DividerItemDecoration(Context context, int orientation,int space,int itemWidth) {
        this.setOrientation(orientation);
        //获取xml配置的参数
        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        this.mContext = context;
...
//        ZALog.e("mDividerHeight", mDividerHeight + "===================");
//        //抗锯齿画笔
//        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
...
//            throw new IllegalArgumentException("invalid orientation");
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        switch (mOrientation) {
            case BOTH_SET:
                //获取Layout的相关参数
                outRect.set((int) ((itemPosition % spanCount) / (spanCount * 1.0f) * mDividerHeight), 0, (int) ((1 - ((itemPosition % spanCount) + 1) / (spanCount * 1.0f)) * mDividerHeight),mDividerHeight);
                break;
            case HORIZONTAL_LIST:
                //水平布局右侧留Margin,如果是最后一列,就不要留Margin了
                outRect.set((int) ((itemPosition % spanCount) / (spanCount * 1.0f) * mDividerHeight), 0, (int) ((1 - ((itemPosition % spanCount) + 1) / (spanCount * 1.0f)) * mDividerHeight),0);
            ...
            case SPE_LIST:
                //垂直布局底部留边，最后一行不留
                if (itemPosition == 0){
                    outRect.set((DisplayUtil.getDisplay(mContext).widthPixels -itemWidth )/2 , 0, 0, 0);
                }else {
